...
def get_client_secret(acc):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(sql)
            secret_path = cursor.fetchall()

            if not secret_path:
                print("Secret path not found for account:", account_name)
                return None
            
            AZURE_CLIENT_SECRET_PATH = secret_path[0]['AZURE_CLIENT_SECRET_PATH_KV2']
            AZURE_CLIENT_ID = secret_path[0]['AZURE_CLIENT_ID']
            ...
            return AZURE_CLIENT_SECRET, AZURE_CLIENT_ID
    finally:
        connection.close()
...
for x in json_data:
    ...
    account_secret, azure_client_id = get_client_secret(account_name)
