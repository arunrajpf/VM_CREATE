...
role_id=os.environ['client_id']
secret_id=os.environ['client_secret']
...
def get_client_secret(acc):
    ...
    try:
        with connection.cursor() as cursor:
            sql = str("SELECT AZURE_CLIENT_SECRET_PATH_KV2, AZURE_CLIENT_ID from ccp.account WHERE account_name='" + acc + "'")
            cursor.execute(sql)
            result = cursor.fetchone()

            secret_path = result['AZURE_CLIENT_SECRET_PATH_KV2']
            client_id = result['AZURE_CLIENT_ID']

            if not secret_path:
                print("Secret path not found for account:", account_name)
                return None
            AZURE_CLIENT_SECRET_PATH = secret_path
            ...
            # Vault URL for revoke token
           # vault_url = "https://or.vault.comcast.com/v1/auth/token/revoke-self"
            # Set the headers
             ...
             #       print(
              #          'error performing token check: failed to persist lease entry: cannot write to readonly storage')
               #     print('sleeping for 5 minutes')
                ...
                #else:
               #     print(resp)
            return AZURE_CLIENT_SECRET, client_id
    finally:
        connection.close()
        ...
        for z in cmi['data']:
            ...
            if 'RHEL 8' in os_name:
                if 'Comcast-RHEL-8-' in z:
                    dates.append(z)
            if 'RHEL 9' in os_name:
                if 'Comcast-RHEL-9-' in z:
                    dates.append(z)
...
for x in json_data:
    ...
    account_secret, client_id = get_client_secret(account_name)
    body = {
