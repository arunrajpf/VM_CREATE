...
excel_data_fragment = pandas.read_excel('ccp_create_server_list_test.xlsx', sheet_name='Input')
...
for x in json_data:
    ...
    if x['DC'] == 'as':
        f.write(
            'url = "https://api.xcloud.comcast.com/api/terraformassist/v1/vRA/ashburn/' + az + '/2cc74b51-2328-4d68-b1a3-eb1b051ff5f1"\n')
    elif x['DC'] == 'ho':
        f.write(
            'url = "https://api.xcloud.comcast.com/api/terraformassist/v1/vRA/hillsboro/' + az + '/2cc74b51-2328-4d68-b1a3-eb1b051ff5f1"\n')
    elif x['DC'] == 'ch' or x['DC'] == 'ch2':
        f.write(
            'url = "https://api.xcloud.comcast.com/api/terraformassist/v1/vRA/chicago/' + az + '/2cc74b51-2328-4d68-b1a3-eb1b051ff5f1"\n')
    f.write('''insecure = "false"
project = "BSDCORE-non-prod-2cc74b51-2328-4d68-b1a3-eb1b051ff5f1"\n''')
    now = datetime.now()
    current_time = now.strftime("%Y%m%d%H%M%S")
    f.write('deployment_name = "BSDCORE-non_prod-' + current_time + '"\n')
    ...
    f.write('rail = "' + x['Rail'] + '"\n')
    f.write('tags = "itrc_application:' + str(x['itrc_application_id']) + ',itrc_environment:' + x['Envt'] + ',itrc_tier:' + x['Tier'] + ',comcastapplicationenvironment:' + x['Envt'] + ',comcastiopapplicationid:' + str(x['IOP App ID']) + ',dns:' + 'true' + ',cada_hostgroup:' + 'bsd_core_hg' + '"\n')
    f.write('cmi = "' + cmi_loc + '"\n' )
...
''')
    ...
    ccp_hostname = list(y['outputs']['vm_name_with_ip']['value'].keys())[0]
    if 'g' in str(x['swap']):
        swap = str(x['swap'])
    else:
        swap = str(x['swap']) + 'g'
    ...
    hosts.write(' ccp_hostname=' + ccp_hostname + '.sys.comcast.net\n')
