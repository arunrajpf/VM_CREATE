...
#     cmi_loc = cmi[0]
    if 'centos' in x['OS'].lower():
        cmi_loc = 'Comcast-CentOS-7-20210910'
    if 'rhel' in x['OS'].lower():
        cmi_loc = 'Comcast-RHEL-7-20210910'
    if x['DC'] == 'as':
        f.write(
...
''')
    ...
    if x['Tier'].lower() == 'app' or x['Tier'].lower() == 'web':
        hosts.write(host_ip + ' swap_size=' + swap + ' filesystem_name=teoco hostname=' + x['Hostname'] + '.sys.comcast.net')
    if x['Tier'].lower() == 'db':
